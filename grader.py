...
def run_test(suite_index, test_index):
    start = datetime.now()
    try:
        result = subprocess.run(["racket", "-l", "racket", "-t", f'{repo_name}/{assignment_name}/testcode.rkt', "-e", f'(call-with-output-file "out.txt" (lambda (out) (write (individual-test {suite_index} {test_index}) out)))'], capture_output=True, timeout=timeout)
    except subprocess.TimeoutExpired:
...
def get_weights():
    weights_array = []
    subprocess.run(["racket", "-l", "racket", "-t", f'{repo_name}/{assignment_name}/testcode.rkt', "-e", f'(call-with-output-file "out.txt" (lambda (out) (write (get-weights) out)))'], capture_output=True)
